    ...
    <div class="valka-app">
      ...
      <section class="main-actions">
        <div class="actions-container">
          <!-- Hacer Cuentas (Generar Factura) -->
          <div v-if="admin.permiso_generar_facturas">
          ...
          <div v-else class="permission-blocked">
            ...
            <p class="permission-message">No tienes permiso para generar facturas. Contacta al dueño.</p>
          </div>

          <!-- Agregar Personal (solo si es burdel) -->
          <div v-if="barTipo === 'burdel'">
            <div v-if="admin.permiso_agregar_personal_femenino">
              <button class="btn btn-action btn-add-staff" @click="showAddStaff">
                <div class="btn-content">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div v-else class="permission-blocked">
              ...
              <p class="permission-message">No tienes permiso para agregar personal femenino. Contacta al dueño.</p>
            </div>
          </div>

          <!-- Cargar productos (Agregar/Sumar inventario) -->
          <div v-if="admin.permiso_agregar_inventario">
          ...
          <div v-else class="permission-blocked">
            ...
            <p class="permission-message">No tienes permiso para agregar o sumar al inventario. Contacta al dueño.</p>
          </div>
        </div>
      </section>

      <!-- =================================== -->
      <!-- SECCIÓN DE PRODUCTOS (solo si tiene permiso) -->
      <!-- =================================== -->
      <section v-if="admin.permiso_ver_productos" class="products-section" @scroll="handleScroll">
      ...
      <!-- Mensaje bloqueado si NO tiene permiso para ver productos -->
      <section v-else class="permission-blocked-full">
        <svg width="80" height="80" viewBox="0 0 24 24" fill="none" stroke="#a0aec0" stroke-width="2">
          <rect x="3" y="11" width="18" height="11" rx="2" ry="2" />
          <path d="M7 11V7a5 5 0 0110 0v4" />
        </svg>
        <h3>No tienes acceso a los productos</h3>
        <p>No tienes permiso para ver los productos del bar. Contacta al dueño para que te otorgue este permiso.</p>
      </section>
  ...
  // -------------------------------------------------
  // Funciones de carga de productos (protegida por permiso)
  // -------------------------------------------------
  const fetchProducts = async (reset = false) => {
    // Solo cargar si tiene permiso
    if (!admin.value.permiso_ver_productos) {
      console.warn('Usuario sin permiso para ver productos')
      return
    }
  ...
  // -------------------------------------------------
  // Scroll infinito (solo si tiene permiso)
  // -------------------------------------------------
  const handleScroll = (e) => {
    if (!admin.value.permiso_ver_productos) return
  ...
  // -------------------------------------------------
  // Navegación y acciones principales (protegidas)
  // -------------------------------------------------
  const goBack = () => {
    adminStore.logout()
    router.push('/login')
  ...
  const makeCounts = () => {
    if (!admin.value.permiso_generar_facturas) {
      Swal.fire({
        icon: 'warning',
        title: 'Permiso requerido',
        text: 'No tienes permiso para generar facturas. Contacta al dueño.',
        customClass: { popup: 'swal2-dark-popup' }
      })
      return
    }
    router.push({ name: 'factura_admin' })
  }

  const openInventoryModal = () => {
    if (!admin.value.permiso_agregar_inventario) {
      Swal.fire({
        icon: 'warning',
        title: 'Permiso requerido',
        text: 'No tienes permiso para modificar el inventario. Contacta al dueño.',
        customClass: { popup: 'swal2-dark-popup' }
      })
      return
    }
  ...
  const handleInventoryModalClose = () => {
    ...
    if (admin.value.permiso_ver_productos) {
      fetchProducts(true) // recarga solo si puede ver productos
    }
  }
  ...
  // -------------------------------------------------
  // Modal agregar personal (protegido)
  // -------------------------------------------------
  const showAddStaff = () => {
    if (!admin.value.permiso_agregar_personal_femenino) {
      Swal.fire({
        icon: 'warning',
        title: 'Permiso requerido',
        text: 'No tienes permiso para agregar personal femenino. Contacta al dueño.',
        customClass: { popup: 'swal2-dark-popup' }
      })
      return
    }
  ...
  const addStaff = async () => {
    if (!admin.value.permiso_agregar_personal_femenino) return
  ...
  * {
  ...
  .permission-blocked-full svg {
    margin-bottom: 1.5rem;
    opacity: 0.6;
  }

  .permission-blocked-full h3 {
    font-size: 1.6rem;
    color: #e2e8f0;
    margin-bottom: 1rem;
  }

  .permission-blocked-full p {
    font-size: 1.1rem;
    max-width: 500px;
  }
  </style>